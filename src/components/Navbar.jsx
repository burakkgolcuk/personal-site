...
export default function Navbar() {
  ...
  const NavLinks = ({ vertical = false, onClick = () => {} }) => (
    <div className={`items-center ${vertical ? "flex flex-col gap-4 py-4" : "hidden md:flex gap-6"}`}>
      <button
        onClick={() => { scrollToId("skills"); onClick(); }}
        className={`text-[18px] leading-7 font-medium ${active === "skills" ? "underline underline-offset-4" : "muted"}`}
        aria-current={active === "skills" ? "page" : undefined}
      >
        {t("nav.skills")}
      </button>
      <button
        onClick={() => { scrollToId("projects"); onClick(); }}
        className={`text-[18px] leading-7 font-medium ${active === "projects" ? "underline underline-offset-4" : "muted"}`}
        aria-current={active === "projects" ? "page" : undefined}
      >
        {t("nav.projects")}
      </button>
      <a href="#contact" onClick={onClick} className="btn btn-line w-[131px] h-[52px]">
        {t("buttons.hire")}
      </a>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="container-1140 h-[62px] flex items-center justify-between">
        {/* Logo (B harfi) */}
        <button onClick={() => scrollToId("home")} aria-label="Go to home" className="relative w-[63.68px] h-[62px]">
          ...
          <span
            ...
            style={{ color: "rgb(123 97 255)", transform: "matrix(0.87,0.49,-0.51,0.86,0,0)" }}
          >
            B
          </span>
        </button>
      ...
      {/* Mobile menu */}
      {open && (
        <div id="mobile-nav" className="md:hidden border-t" style={{ borderColor: "rgb(var(--border))" }}>
